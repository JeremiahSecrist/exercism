...
pub fn root_for_team(team: Team) -> Bool {
  case team {
    t if t.coach.name == "Gregg Popovich" -> True
    t if t.coach.former_player -> True
    t if t.name == "Chicago Bulls" -> True
    t if t.stats.wins >= 60 -> True
    t if t.stats.wins < t.stats.losses -> True
    _ -> False
